...
const playbooks = [
  ...
  table([2700, 2660, 4000], [
    ...
    ["KOI script package(s)", "Action Center → Scripts Library", "Upload the KOI deployment script(s) yourself — they are NOT part of this pack. The Library name must match the List's script.name exactly (e.g., \"KOI Deployment Script - Windows\"). To make the reference rename-proof, pin script.uuid instead."],
    ...
    ["Target agents", "Endpoints", "Cortex agents installed on the targets, CONNECTED and not isolated, with an OS matching the entry's endpoint_os — otherwise the entry reports skipped."],
