...
const playbooks = [
  h1("10. KOI Script Runner Playbooks"),
  ...
  table([3400, 5960], [
    ...
    ["KOI Script Runner - Refresh Entry", "Per entry: walks the endpoint group (paged past 100 via the Core REST API) and append-only upserts membership into the scope's tracker List — never touching an existing last_scan."],
  ...
  table([2700, 2660, 4000], [
    ...
    ["Configuration List", "Settings → Object Setup → Lists", "A JSON List named exactly \"Koi Script Runner\". The pack ships a ready-made sample at Lists/list-Koi_Script_Runner.json — copy its data value and edit the script name and endpoint group (section 10.3)."],
  ...
  p("Create a JSON List named exactly \"Koi Script Runner\" (Settings → Configurations → Object Setup → Lists) containing an array of entries. The pack ships a ready-made one at Lists/list-Koi_Script_Runner.json — copy its data value rather than writing the structure by hand, then edit the two fields that are specific to you: script.name (the KOI deployment script exactly as it appears in Action Center) and target.endpoint_groups (your group). Everything else already carries working defaults:"),
  code('['),
  code('  {'),
  code('    "disabled": false,'),
  ...
  step_playbooks([{ text: "Refresh Job. ", bold: true }, { text: "Time-triggered, infrequent (e.g., hourly), playbook " }, { text: "KOI Script Runner - Refresh Job", bold: true }, { text: ". Run this at least once before the first Scan so the tracker is populated." }]),
  step_playbooks([{ text: "Scan Job. ", bold: true }, { text: "Time-triggered, frequent (e.g., every 10 minutes), playbook " }, { text: "KOI Script Runner - Scan Job", bold: true }, { text: ". Save, then use Run now for an immediate first run." }]),
...
const triagePlaybooks = [
  ...
  p("A second, independent playbook set turns each KOI alert into an investigated, classified and — when you approve it — actioned case. Attach KOI IR - Alert Triage to your KOI alerts and the rest runs automatically."),
  ...
  table([3100, 6260], [
    ...
    ["KOI IR - Extract Alert Context", "Sub. Parses the koi_context JSON embedded in the alert description into the KoiContext object."],
  ...
  step_triagePlaybooks([{ text: "Attach " }, { text: "KOI IR - Alert Triage", bold: true }, { text: " as the playbook for KOI alerts, or run it on an existing alert from the CLI:" }]),
  code('!setPlaybook playbookId="KOI IR - Alert Triage"'),
  ...
  p("KOI IR - Investigate Item, KOI IR - Investigate Device and KOI IR - Block and Remediate declare required inputs, so they are designed to be called as sub-playbooks (or invoked by an analyst supplying those inputs) rather than attached directly to an alert.", { italics: true, color: GRAY }),
];
