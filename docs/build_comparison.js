...
const decision = [
  ...
  table([2400, 3480, 3480], [
    ...
    ["Device investigation", "Full — KOI's own device records", "Partial — inferred from inventory and XDR data"],
...
const customOnly = [
  ...
  table([2700, 3200, 3460], [
    ...
    ["Device-centred investigation", "koi-devices-list, koi-device-inventory-get, koi-groups-list",
     "Read KOI's own device record — last seen, registration, logged-on user, status — and everything KOI inventoried on that host. Option B can only infer a host view by filtering inventory and correlating XDR data."],
  ...
  bullet("Device investigation is first-hand. An alert on a host expands into KOI's own device record and everything KOI inventoried there, rather than an inventory query filtered by host."),
...
const decide = [
  ...
  table([4200, 2580, 2580], [
    ...
    ["Do analysts need KOI's own device record, not just an inventory view?", "Option A — the device commands are custom-only", "Either"],
