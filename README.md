...
- **The List** — named exactly `Koi Script Runner`. The pack ships one. To create it by hand, copy the ready-to-paste block in [`Lists/README.md`](Lists/README.md) — not the raw content item, whose `data` field is escaped — then edit two things — `script.name` (the KOI deployment script as it appears in Action Center) and `target.endpoint_groups` (your group). Everything else has working defaults
